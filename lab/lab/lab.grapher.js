...
(function(factory) {
  ...
  if (true) {
    !(__WEBPACK_AMD_DEFINE_ARRAY__ = [__webpack_require__(5), __webpack_require__(57), exports], __WEBPACK_AMD_DEFINE_RESULT__ = (function(_, $, exports) {
...
$.fn.truncate = function (maxWidth) {
  ...
  if (width > maxWidth) {
    ...
    $el.attr("title", $el.text()); // Update original element.
...
var BarGraphModel = backbone__WEBPACK_IMPORTED_MODULE_0___default.a.Model.extend({
  defaults: {
...
/* harmony default export */ __webpack_exports__["a"] = (BarGraphModel);
...
"use strict";
/* harmony import */ var common_jquery_plugins__WEBPACK_IMPORTED_MODULE_0__ = __webpack_require__(33);
/* harmony import */ var common_jquery_plugins__WEBPACK_IMPORTED_MODULE_0___default = /*#__PURE__*/__webpack_require__.n(common_jquery_plugins__WEBPACK_IMPORTED_MODULE_0__);
...
getRealWidth = function getRealWidth(d3selection) {
...
CANONICAL_FONT_SIZE = 16,
    getScaleFunc = function getScaleFunc(fontSize) {
  var factor = fontSize / CANONICAL_FONT_SIZE;
  return function (val) {
    return val * factor;
  };
},
    setupValueLabelPairs = function setupValueLabelPairs(yAxis, ticks) {
  var values = [],
      labels = {},
      i,
      len;
  ...
  yAxis.tickValues(values).tickFormat(function (value) {
    return labels[value];
  });
},
    getFormatFunc = function getFormatFunc(formatString, unitsString) {
  var format = d3.format(formatString);
  return function (value) {
    return format(value) + " " + unitsString;
  };
},
    BarGraphView = backbone__WEBPACK_IMPORTED_MODULE_1___default.a.View.extend({
  // Container is a DIV.
  tagName: "div",
  className: "bar-graph",
  initialize: function initialize() {
    ...
    // gradient names.
    this.uid = getUID();
    this.$topArea = $('<div class="top-area">').appendTo(this.$el); // Create some SVG elements, which are constant and doesn't need to
    ...
    this.scale = null;
    this.barWidth = null;
    this.$bottomArea = $('<div class="bottom-area">').appendTo(this.$el); // Register callbacks!
  ...
  render: function render() {
    ...
    paddingTop = renderLabels ? scale(8) : scale(3),
        ...
        offset = 0; // Set height of the most outer container.

    this.$el.outerHeight(options.height);

    this._setupHorizontalTitle();

    this.svgHeight = this.$el.height() - this.$topArea.height() - this.$bottomArea.height(); // Setup SVG element.
    ...
    this.yScale.domain([options.min, options.max]).range([this.svgHeight - paddingTop, paddingBottom]).clamp(true); // Setup scale used to translation of the bar height.

    this.heightScale.domain([options.min, options.max]).range([0, this.svgHeight - paddingTop - paddingBottom]).clamp(true); // Render elements from left to right.

    this.axisContainer.selectAll("*").remove();

    if (renderLabels) {
      // Setup Y axis.
      this.yAxis.scale(this.yScale).tickValues(null).tickPadding(0).tickSize(0, 0, 0).orient("left");

      if (typeof options.labels === "number") {
        // Just normal tics.
        this.yAxis.ticks(options.labels).tickFormat(getFormatFunc(options.labelFormat, options.units));
      } else {
    ...
    this.traingle.classed("triangle", true).attr({
    ...
    offset = this._setupTitle(offset); // Convert final width in px into value in ems.
    // That ensures that the SVG will work well with semantic layout.

    this.vis.attr("width", offset / fontSize + "em");
    this.$el.css("min-width", offset / fontSize + "em"); // work-around bug on iPad2 where container is not expanding in width
    ...
    this.$el.css("min-width", offset / fontSize + 0.8 + "em"); // Finally, update displayed values.
  ...
  update: function update() {
    ...
    this.bar.attr("height", this.heightScale(value)).attr("y", this.yScale(value));
  ...
  modelChanged: function modelChanged() {
    var changedAttributes = this.model.changedAttributes(),
        ...
        valChanged,
        secValChanged,
        name; // There are two possible cases:
    ...
    secValChanged = typeof changedAttributes.secondValue !== 'undefined'; // Case 2. 1 or 2 attributes have changed, check if they are "value" and "secondValue".

    if (count === 1 && (valChanged || secValChanged) || count === 2 && valChanged && secValChanged) {
      this.update();
    } else {
      this.render();
    }
  },
  _getBarGradient: function _getBarGradient(color) {
    ...
    if (gradient.empty()) {
      ...
      gradient = this.defs.append("linearGradient").attr("id", id).attr("x1", "0%").attr("y1", "0%").attr("x2", "0%").attr("y2", "100%");
    } else {
      gradient.selectAll("stop").remove();
    }

    gradient.append("stop").attr("stop-color", color.brighter(2).toString()).attr("offset", "0%");
    gradient.append("stop").attr("stop-color", color.toString()).attr("offset", "100%");
    return "url(#" + id + ")";
  },
  _getFillGradient: function _getFillGradient(color) {
    ...
    gradient.append("stop").attr("stop-color", color).attr("offset", "0%");
    gradient.append("stop").attr("stop-color", color).attr("stop-opacity", 0.5).attr("offset", "15%");
    gradient.append("stop").attr("stop-color", color).attr("stop-opacity", 0.4).attr("offset", "100%");
    return "url(#" + id + ")";
  },
  _setupGrid: function _setupGrid(offset) {
    var gridLines = this.yScale.ticks(this.model.get("gridLines")),
        yScale = this.yScale,
        width = this.barWidth; // Remove first and last tick, as we don't want to draw it as grid line.

    gridLines.pop();
    gridLines.shift();
    this.grid = this.gridContainer.selectAll(".grid-line").data(gridLines, String), this.grid.enter().append("path").attr("class", "grid-line");
  ...
  _setupTitle: function _setupTitle(offset) {
    ...
    if (title && this.model.get("titleOn") === "right") {
      ...
      titleG.each(function (d, i) {
        ...
        g.select("text").text(self._processTitle(d)).attr("dy", -(lines - i - 1) + "em");
      }); // Transform whole container.

      this.titleContainer.attr("transform", "translate(" + offset + ", " + this.svgHeight / 2 + ") rotate(90)"); // Update offset.
  ...
  _setupHorizontalTitle: function _setupHorizontalTitle() {
    // "title" option is expected to be string
    // or array of strings.
    var title = this.model.get("title"),
...
/* harmony default export */ __webpack_exports__["a"] = (BarGraphView);
...
/* WEBPACK VAR INJECTION */}.call(this, __webpack_require__(15), __webpack_require__(14)(module)))
...
/* harmony import */ var lab_config__WEBPACK_IMPORTED_MODULE_0__ = __webpack_require__(4);
/* harmony import */ var grapher_bar_graph_bar_graph_model__WEBPACK_IMPORTED_MODULE_1__ = __webpack_require__(34);
/* harmony import */ var grapher_bar_graph_bar_graph_view__WEBPACK_IMPORTED_MODULE_2__ = __webpack_require__(35);
/* harmony import */ var lab_grapher__WEBPACK_IMPORTED_MODULE_3__ = __webpack_require__(22);
/* harmony import */ var lab_grapher__WEBPACK_IMPORTED_MODULE_3___default = /*#__PURE__*/__webpack_require__.n(lab_grapher__WEBPACK_IMPORTED_MODULE_3__);



 // Create or get 'Lab' global object (namespace).

window.Lab = window.Lab || {}; // Export this API under 'grapher' name.

window.Lab.grapher = window.Lab.grapher || {};
window.Lab.grapher.BarGraphModel = grapher_bar_graph_bar_graph_model__WEBPACK_IMPORTED_MODULE_1__[/* default */ "a"];
window.Lab.grapher.BarGraphView = grapher_bar_graph_bar_graph_view__WEBPACK_IMPORTED_MODULE_2__[/* default */ "a"];
window.Lab.grapher.Graph = lab_grapher__WEBPACK_IMPORTED_MODULE_3___default.a; // Export config modules.

window.Lab.config = lab_config__WEBPACK_IMPORTED_MODULE_0__[/* default */ "a"]; // Also return public API as module.

/* harmony default export */ __webpack_exports__["default"] = (window.Lab.grapher);

/***/ })

/******/ });
//# sourceMappingURL=lab.grapher.js.map